...
Para possibilitar a realização das buscas para os pontos de monitoramento, é necessário ter informações sobre os pontos de monitoramento. A função [`get_sampling_points()`](https://github.com/beatrizmilz/infoaguas/blob/master/R/get_sampling_points.R) realiza um Web Scraping e retorna uma base contendo informações sobre os pontos. Obs: Essa base será descrita posteriormente. Não é necessário informar argumentos para essa função.
