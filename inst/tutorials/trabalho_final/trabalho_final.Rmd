...
dados_billings <- readr::read_delim("dados_billings.csv", 
    ";", escape_double = FALSE, col_types = cols(valor_texto = col_character()), 
    trim_ws = TRUE)
```
...
- `latitude` e `longitude`: Latitude e Longitude do ponto de coleta.
...
dados_billings %>% 
  ...
  leaflet::leaflet() %>% 
  addTiles() %>% 
  addMarkers(~longitude_decimal,  ~latitude_decimal)
data(quakes)
```
...
Ao final, para exemplificar o uso dos dados obtidos, __uma breve análise descritiva será realizada com os dados de qualidade da água no território da Macrometrópole Paulista__.
